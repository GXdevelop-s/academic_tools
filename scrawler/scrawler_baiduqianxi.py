...
CitiesCode = {
    ...
    '哈尔滨市':230100,'齐齐哈尔市':230200,'鸡西市':230300,'鹤岗市':230400,'双鸭山市':230500,'大庆市':230600,'伊春市':230700,'佳木斯市':230800,'七台河市':230900,'牡丹江市':231000,'黑河市':231100,'绥化市':231200,'大兴安岭地区':232700,'上海市':310000,
    ...
    '海口市':460100,'三亚市':460200,'三沙市':460300,'儋州市':460400,'五指山市':469001,'琼海市':469002,'文昌市':469005,'万宁市':469006,'东方市':469007,'定安县':469021,'屯昌县':469022,'澄迈县':469023,'临高县':469024,'白沙黎族自治县':469025,'昌江黎族自治县':469026,'乐东黎族自治县':469027,'陵水黎族自治县':469028,'保亭黎族苗族自治县':469029,'琼中黎族苗族自治县':469030,'重庆市':500000,
...
# 台湾省搜不到数据，因此新建一个城市编码字典Cities————暂时没用
Cities = {
    '北京市':110000,'天津市':120000,
    '石家庄市':130100,'唐山市':130200,'秦皇岛市':130300,'邯郸市':130400,'邢台市':130500,'保定市':130600,'张家口市':130700,'承德市':130800,'沧州市':130900,'廊坊市':131000,'衡水市':131100,
    '太原市':140100,'大同市':140200,'阳泉市':140300,'长治市':140400,'晋城市':140500,'朔州市':140600,'晋中市':140700,'运城市':140800,'忻州市':140900,'临汾市':141000,'吕梁市':141100,
...
def migration_index(FileTittle, classname, direction, CodeDict):     # CodeDict字典里所有城市的迁徙规模指数，以全国列表形式列出
    ...
    # 创建一个表格workbook  FileTittle：城市\省份\全国
    workbook = xlsxwriter.Workbook(f'{FileTittle}{nameofdire}规模指数.xlsx')
    worksheet = workbook.add_worksheet('Sheet')             # 创建表格中的第一张表
    ...
    for key, value in CodeDict.items():
        worksheet.write(times, 0, str(value))                # 写入城市代码--------表格第0列
        worksheet.write(times, 1, str(key))                  # 写入城市名即关键词---表格第1列
    ...
    for Area, Code in CodeDict.items():
        ...
        print(f'{Area}:{url}')
        response = requests.get(url, timeout=100)               # 发出请求并json化处理
        # time.sleep(1)
        ...
        if data_dict['errmsg'] == 'SUCCESS':
            ...
            datelist = []
            for date, index in data_list.items():             # 按日期排序 从数据列表中筛选出来“日期”和“指标”
                datelist.append(date)                         # 将每个日期写入日期列表中
            datelist.sort()                                   # 日期列表排序
            for date in datelist:
                index = data_list[date]
                # print(f'{date} : {index}')
                worksheet.write(0, counter_date, float(date))   # 将日期写入第0行表头
...
def internalflow_all_date(CodeDict):        # CodeDict字典里所有城市的迁徙规模指数，以全国列表形式列出
    # 创建一个workbook
    workbook = xlsxwriter.Workbook('20220527上班和休闲指数.xlsx')    # 创建一个workbook 设置编码
    worksheet1 = workbook.add_worksheet('上班出行强度指数')    # 创建一个worksheet
    worksheet2 = workbook.add_worksheet('就餐休闲出行强度指数')    # 创建一个worksheet
    # 写入行头各城市代码及其城市名
    citiesorder = {}                                 # 存放城市序号的空字典
    worksheet1.write(0, 0, '城市代码')                # 写入行头
    worksheet1.write(0, 1, '城市')                    # 写入行头
    worksheet2.write(0, 0, '城市代码')                # 写入行头
    worksheet2.write(0, 1, '城市')                    # 写入行头
    times = 1
    for key, value in CodeDict.items():
        worksheet1.write(times, 0, str(value))                # 写入城市代码
        worksheet1.write(times, 1, str(key))                  # 写入城市名
        worksheet2.write(times, 0, str(value))                # 写入城市代码
        worksheet2.write(times, 1, str(key))                  # 写入城市名
        citiesorder[str(key)] = times                         # 写入城市序号字典
        times += 1
    # 开始抓取数据
    for Area, Code in CodeDict.items():
        # url = f'http://huiyan.baidu.com/migration/odsemantic.jsonp?dt=city&id={Code}&date=0'
        url=f'http://huiyan.baidu.com/migration/odsemantic.jsonp?dt=city&id={Code}&date=0'
        print(f'{Area}:{url}')
        try:
            response = requests.get(url, timeout=100)    # #发出请求并json化处理
            r = response.text[4:-1]                     # 去头去尾
            data_dict = json.loads(r, strict=False)     # 字典化
        except:
            print('正在准备重新启动------------------------------------------\n')
            time.sleep(15)
            response = requests.get(url, timeout=1000)    # #发出请求并json化处理
            r = response.text[4:-1]                     # 去头去尾
            data_dict = json.loads(r, strict=False)     # 字典化
            print('启动中----------------------------------------------------\n')
        time.sleep(0.1)  #挂起一秒
        ...
        data_dict = json.loads(r)                       # 字典化
        if data_dict['errmsg'] == 'SUCCESS':
            work_data_list = data_dict['data']['list']['work']
            entert_data_list = data_dict['data']['list']['entertainment']
            counter_date = 2                            # 日期计数器
            work_datelist = []
            entert_datelist = []
            for date, index in work_data_list.items():          # 按日期排序
                work_datelist.append(date)
            for date, index in entert_data_list.items():        # 按日期排序
                entert_datelist.append(date)
            work_datelist.sort()
            entert_datelist.sort()
            for date in work_datelist:
                index_w = work_data_list[date]
                index_e = entert_data_list[date]
                # print(f'{date} : {index}')
                worksheet1.write(0, counter_date, float(date))
                worksheet2.write(0, counter_date, float(date))
                worksheet1.write(citiesorder[str(Area)], counter_date, float(index_w))
                worksheet2.write(citiesorder[str(Area)], counter_date, float(index_e))
                counter_date += 1                               # 日期计数器自加一
        else:
            print('错误')
    workbook.close()
...
# migration_index('全国', 'country', 'out', quanguo)
